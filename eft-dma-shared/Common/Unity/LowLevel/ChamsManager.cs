...
namespace eft_dma_shared.Common.Unity.LowLevel
{
    public static class ChamsManager
    {
        ...
        public static int GetMaterialIDForLoot(ChamsMode mode, ChamsEntityType lootType)
        {
            ...
            {
                LoneLogging.WriteLine($"[Chams] Warning: {lootType} is not a valid loot entity type");
                return -1;
            }
            ...
            RemoteBytes chamsColorMem)
        {
            try
            {
                ...
                try
                {
                    var playerChams = chamsConfig.PlayerChams;
                    if (playerChams != null && playerChams.ContainsKey(playerType))
                    {
                        var setting = playerChams[playerType];
                        visibleColorStr = setting?.VisibleColor ?? DefaultVisibleColor;
                        invisibleColorStr = setting?.InvisibleColor ?? DefaultInvisibleColor;
                    }
                }
                catch { }
